...
// Fungsi untuk mendapatkan data limbah berdasarkan userId
export const getLimbahByUserId = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      req.userId !== parseInt(userId) // Pengguna petani hanya dapat mengakses data miliknya
    ) {
      return res.status(403).json({ message: "Anda tidak memiliki izin untuk mengakses data ini." });
    }

    // Temukan data limbah berdasarkan userId
    const limbah = await limbahpetani.findAll({
      where: { userId: userId },
      include: [
        {
          model: User,
    ...
    // Jika data limbah tidak ditemukan
    if (!limbah || limbah.length === 0) {
      return res.status(404).json({ message: "Data limbah tidak ditemukan untuk userId ini." });
    }

    // Kirim data yang diambil sebagai respons
    return res.status(200).json(limbah);
  } catch (error) {
    console.error("Error saat mendapatkan data limbah:", error.message);
